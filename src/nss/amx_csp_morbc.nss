...
Spell Description:
An orb of cold about 3 inches across shoots from your palm at its target dealing
1D6 points of cold damage per caster level (maximum 15D6). You must succeed on
ranged touch attack to hit your target. A creature struck by the orb takes
damage and becomes blinded by the cold for 1 round. A successful fortitude save
negates the blind effect, but does not reduce damage.
...
void main() {
    ...
    if (nTouch >= 1) {
        if (FortitudeSave(oTarget,nDC,SAVING_THROW_TYPE_SPELL,oCaster) < 1) {
            float fDur = 6.0f;
            effect eOrbDebuff = EffectBlindness();
