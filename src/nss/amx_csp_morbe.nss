...
Spell Resistance: No
Spell Description:
An orb of electricity about 3 inches across shoots from your palm at its target
dealing 1D6 points of electricity damage per caster level (maximum 15D6). You
must succeed on ranged touch attack to hit your target. A creature struck by
the orb take damage and becomes entangled by the electricity for 1 round. A
successful fortitude save negates the entangled effect, but does not reduce damage.
...
void main() {
    ...
    if (nTouch >= 1) {
        if (FortitudeSave(oTarget,nDC,SAVING_THROW_TYPE_SPELL,oCaster) < 1) {
            float fDur = 6.0f;
            effect eOrbDebuff = EffectEntangle();
