/*  Trigger :: OnEnter : Evil/Neutral-only Entry Teleporter

    --------
    Verbatim
    --------
    This script will teleport an evil-/neutral-aligned player to some waypoint destination (tag stored on the trigger itself).
...
void main( ){
    ...
    // Destination and teleporting player valid; and player has an evil/neutral alignment, then teleport the player
    if( GetIsObjectValid( oDest ) && GetIsPC( oPC ) && ( nAlignment == ALIGNMENT_EVIL || nAlignment == ALIGNMENT_NEUTRAL || nAlignment == ALIGNMENT_GOOD ) ){
