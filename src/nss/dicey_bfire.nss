...
//:://////////////////////////////////////////////
//Dicey Addition - Upped reflex save to make it more effective to mobs. Also added piercing dmg to targets. And fixed to VFX to make it look cooler.
   ...
   int iDC = 35;              //Difficulty check(reflex save for half damage)
   float fSize = 5.0f;        //Radius of damage area
   int iDice = 5;             //Number of dice to roll for damage (currently d8s, changeable in code)
...
void ApplyDamage()
{
 ...
 eBolt = EffectVisualEffect(245, FALSE);
 eBlast = EffectVisualEffect(22, FALSE);
 ...
 while(GetIsObjectValid(oTarget))
    {
     ...
     iDamage = d8(iDice); //Set Ballista Damage Here

     eDamP = EffectDamage( iDamage, DAMAGE_TYPE_PIERCING, DAMAGE_POWER_NORMAL);
     eDamF = EffectDamage( iDamage, DAMAGE_TYPE_FIRE, DAMAGE_POWER_NORMAL);
     iDamage = GetReflexAdjustedDamage(iDamage, oTarget, iDC,SAVING_THROW_REFLEX);
        if (iDamage > 0)
          DelayCommand(fDelay,ApplyEffectToObject(DURATION_TYPE_INSTANT, eDamP, oTarget));
