...
void main(){
    ...
    // Unique VFX
    effect eVisual1 = EffectVisualEffect(VFX_DUR_FLAG_RED);
    eVisual1 = SupernaturalEffect(eVisual1);
    effect eVisual2 = EffectVisualEffect(VFX_DUR_SPELLTURNING);
    eVisual2 = SupernaturalEffect(eVisual2);

    ApplyEffectToObject(DURATION_TYPE_PERMANENT,eVisual1,oCritter);
    ApplyEffectToObject(DURATION_TYPE_PERMANENT,eVisual2,oCritter);


    // Anti Magic Aura
    effect eAura = EffectAreaOfEffect(AOE_MOB_DRAGON_FEAR, "djinni_antim_aur", "****", "djinni_antim_ext");
