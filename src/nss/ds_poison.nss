#include "x2_inc_switches"



void main(){

    //event variables
    int nEvent  = GetUserDefinedItemEventNumber();
    int nResult = X2_EXECUTE_SCRIPT_END;

    object oPC;
    object oItem;
    object oTarget;
    string sItemName;
    int nPoison;
    effect ePoison;


    switch (nEvent){

        case X2_ITEM_EVENT_ONHITCAST:

            // item activate variables
            oPC       = OBJECT_SELF;
            oItem     = GetSpellCastItem();
            oTarget   = GetSpellTargetObject();
            nPoison   = GetLocalInt(oItem, "ds_poison");
            ePoison   = EffectPoison( (59+nPoison) );


            //testing
            //SendMessageToPC( GetFirstPC(),IntToString(nPoison));
            //SendMessageToPC( GetFirstPC(),GetName(oItem));



            ApplyEffectToObject(DURATION_TYPE_TEMPORARY,ePoison,oTarget,30.0f);

        break;

        case X2_ITEM_EVENT_ACTIVATE:

            // item activate variables
            oPC       = GetItemActivator();
            oItem     = GetItemActivated();
            nPoison   = GetLocalInt(oItem, "ds_poison");
            ePoison   = EffectPoison( (59+nPoison) );

            ApplyEffectToObject(DURATION_TYPE_TEMPORARY,ePoison,oPC,60.0f);

        break;

    }
    //Pass the return value back to the calling script
    SetExecutedScriptReturnValue(nResult);
}

