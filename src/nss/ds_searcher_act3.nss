...
void main(){
     ...
     //set item
     SetLocalString( oPC, "ds_item", "DCrod" );
...
void SetCharSelection( object oPC, int nNode ){
     ...
     while ( GetIsObjectValid( oOnlinePC ) == TRUE ){
          ...
          string sSuggested = GetLocalString( oOnlinePC, "dc_recommended" );

          //select correct PCs
          if ( GetPlayersInSection( oOnlinePC, nNode ) == TRUE && nSuggested > 0 ){

               nSelectedPCs++;

               SetLocalObject( oPC, "ds_pc_"+IntToString( nSelectedPCs ), oOnlinePC );
               SetLocalInt( oPC, "ds_check_"+IntToString( nSelectedPCs ), 1 );
               SetCustomToken( ( 4000 + nSelectedPCs ), GetName ( oOnlinePC )+" ("+IntToString( nSuggested )+", "+sSuggested+")" );
          }
