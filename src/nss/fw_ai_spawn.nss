// FW modified AI_SPAWN
// 12-15-2015
// 03-08-2015   enabled variable settings for ominvore and herbivore
...
void main()
{
    int iWalkFrequency = GetLocalInt(OBJECT_SELF, "walk_wait");
    int iPheno = GetLocalInt(OBJECT_SELF, "fw_pheno");
    if( iPheno > 0 )  {
        SetPhenoType( iPheno, OBJECT_SELF );
    }

    // last param turn off ai
    int iDisabled = GetLocalInt(OBJECT_SELF, "fw_disabled");
    if( iDisabled > 0 )
    {
        return;
    }
...
*/
    ...
    int iOmnivore = GetLocalInt(OBJECT_SELF, "fw_omnivore");
    if( iOmnivore > 0 )
    {
    ...
    int iHerbivore = GetLocalInt(OBJECT_SELF, "fw_herbivore");
    if( iHerbivore > 0 )
    {
...
// DEFAULT GENERIC BEHAVIOR (DO NOT TOUCH) *****************************************************************************************
    ...
    WalkWayPoints(FALSE, IntToFloat( iWalkFrequency ) );           // Optional Parameter: void WalkWayPoints(int nRun = FALSE, float fPause = 1.0)
