// script name: fw_fulldynamic
// this script can run a maximum of once a week, having its own internal cooldown
// this script is supposed to load all areas, then destroy all the ones it loaded this way after 60 seconds, then kill the server
...
void IterateAllFiles(string filter, string script){
    ...
    SetLocalInt(OBJECT_SELF, "fulldynloaded", 1);
...
void main()
{
    ...
    if(file != ""){
        ...
        string sPath = NWNX_ReadStringFromINI( "AMIA", "FullDynamicAreaPath", "0", "./nwnx.ini" ) + "*.*";
        IterateAllFiles(sPath,"fw_fulldynamic");
    }
}
