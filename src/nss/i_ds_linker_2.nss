...
void main(){
    ...
    switch (nEvent){
        ...
        case X2_ITEM_EVENT_ACTIVATE:
            ...
            int nStatus      = GetLocalInt( oPC, "ds_linker_2" );
            string sTag1     = "ds_link21_"+GetPCPublicCDKey( oPC );
            string sTag2     = "ds_link22_"+GetPCPublicCDKey( oPC );
            ...
            if ( GetIsObjectValid( oWP1 ) == FALSE ){
                ...
                CreateObject( OBJECT_TYPE_WAYPOINT, "ds_link_2", lTarget, FALSE, sTag1 );
                CreateObject( OBJECT_TYPE_PLACEABLE, "ds_link_2", lTarget, FALSE, sTag2 );
                SetLocalInt( oPC, "ds_linker_2", 1 );
            }
            else if ( GetIsObjectValid( oWP2 ) == FALSE  ){

                SendMessageToPC( oPC, "Creating second portal" );
                CreateObject( OBJECT_TYPE_WAYPOINT, "ds_link_2", lTarget, FALSE, sTag2 );
                CreateObject( OBJECT_TYPE_PLACEABLE, "ds_link_2", lTarget, FALSE, sTag1 );
                SetLocalInt( oPC, "ds_linker_2", 2 );
            }
            else{

                SendMessageToPC( oPC, "Removing portals" );
                DeleteLocalInt( oPC, "ds_linker_2" );
                DestroyObject( oWP1 );
                DestroyObject( oWP2 );
