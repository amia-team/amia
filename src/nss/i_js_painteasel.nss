...
void main(){
            ...
            if (GetObjectType( oTarget ) == OBJECT_TYPE_CREATURE || GetObjectType( oTarget ) == OBJECT_TYPE_DOOR ||
                ...
                SendMessageToPC(oPC,"You can only use this item on the ground to place/remove the easel PLC.");
                return;
            }
            else {
                ds_j_SpawnItem( oPC, oItem, "js_painting", "lala", lTarget, GetName( oItem ), GetDescription( oItem ) );
            }
}
