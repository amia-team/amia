// Item event script for Rodent Empathy. Dominates hostile/neutral rodents
...
void ActivateItem( )
{
    ...
    // Effects to be added to rodents.
    ...
    while( oVictim != OBJECT_INVALID && nCount < 3 ) // Cap targets to 3
    {
        ...
        {
            // Check for rodents only.
            if( GetAppearanceType( oVictim ) >= 386 && GetAppearanceType( oVictim ) <= 387 ||
                GetAppearanceType( oVictim ) >= 1256 && GetAppearanceType( oVictim ) <= 1260 ||
                GetAppearanceType( oVictim ) == 1509 ||
                GetAppearanceType( oVictim ) == 1986 ||
                GetAppearanceType( oVictim ) == 1987 ||
                GetCreatureTailType( oVictim ) == 365 ||
                GetCreatureTailType( oVictim ) == 379 ||
                GetCreatureTailType( oVictim ) >= 910 && GetCreatureTailType( oVictim ) <= 914 ||
                GetCreatureTailType( oVictim ) == 1340 )
            {
                // Only works on non-evil rodents.
