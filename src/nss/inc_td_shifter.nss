...
effect EffectShifterProgressionACEffect( object oPC, effect eEffect ){
    ...
    SendMessageToPC( oPC, "<c � >Bonus AC:" );
...
effect EffectGearToAC( effect eReturn, object oPC ){
    ...
    SendMessageToPC( oPC, "<c � >AC Merge: " );
    ...
    if( nMonkAC && nWisdom > 0 ){
        ...
        SendMessageToPC( oPC, "<c � >Monk AC penalty: "+IntToString( nWisdom ) );
    }

    if( nDeflection > 0 ){

        eReturn = EffectLinkEffects( EffectACIncrease( nDeflection, AC_DEFLECTION_BONUS ), eReturn );
        SendMessageToPC( oPC, "<c � >Deflection: "+IntToString( nDeflection ) );
    }

    if( nNatural > 0 ){

        eReturn = EffectLinkEffects( EffectACIncrease( nNatural, AC_NATURAL_BONUS ), eReturn );
        SendMessageToPC( oPC, "<c � >Natural: "+IntToString( nNatural ) );
    }

    if( nDodge > 0 ){

        eReturn = EffectLinkEffects( EffectACIncrease( nDodge, AC_DODGE_BONUS ), eReturn );
        SendMessageToPC( oPC, "<c � >Dodge: "+IntToString( nDodge ) );
    }

    if( nSheild > 0 ){

        eReturn = EffectLinkEffects( EffectACIncrease( nSheild, AC_SHIELD_ENCHANTMENT_BONUS ), eReturn );
        SendMessageToPC( oPC, "<c � >Shield: "+IntToString( nSheild ) );
    }


    /*SendMessageToPC( oPC, "<c � >Deflection: "+IntToString( nDeflection ) );
    SendMessageToPC( oPC, "<c � >Dodge: "+IntToString( nDodge ) );
    SendMessageToPC( oPC, "<c � >Sheild: "+IntToString( nSheild + nBaseSheild ) );
    SendMessageToPC( oPC, "<c � >Armor: "+IntToString( nBaseArmor + nArmor ) );*/
...
void ShifterMerge( object oPC, object oOldWeapon, object oArmor, object oRing1, object oRing2, object oBoots, object oSheild, object oAmulet, object oCloak, object oHands, object oBelt, object oHelm, int nPolymorph ){
    ...
    if( GetIsObjectValid( oWeaponNew ) && GetIsObjectValid( oOldWeapon ) ){
        ...
        SendMessageToPC( oPC, "<c � >Merging "+GetName( oOldWeapon )+" with "+GetName( oWeaponNew )+"!" );
        ...
        if( GetIsObjectValid( oRClaw ) ){
            SendMessageToPC( oPC, "<c � >Merging "+GetName( oOldWeapon )+" with right claw!" );
            MergeItemProperties( oPC, oOldWeapon, oRClaw );
        }
        if( GetIsObjectValid( oBite ) ){
            SendMessageToPC( oPC, "<c � >Merging "+GetName( oOldWeapon )+" with bite!" );
            MergeItemProperties( oPC, oOldWeapon, oBite );
        }
        if( GetIsObjectValid( oLClaw ) ){
            SendMessageToPC( oPC, "<c � >Merging "+GetName( oOldWeapon )+" with left claw!" );
    ...
    if( StringToInt( Get2DAString( "polymorph", "MergeA", nPolymorph ) ) ){
        ...
        if( GetIsObjectValid( oArmorNew ) ){
            ...
            if( GetIsObjectValid( oArmor ) ){
                SendMessageToPC( oPC, "<c � >Merging "+GetName( oArmor )+" with "+GetInventorySlotName( INVENTORY_SLOT_CARMOUR )+"!" );
                MergeItemProperties( oPC, oArmor, oArmorNew );
            }

            if( GetIsObjectValid( oSheild ) ){
                SendMessageToPC( oPC, "<c � >Merging "+GetName( oSheild )+" with "+GetInventorySlotName( INVENTORY_SLOT_CARMOUR )+"!" );
                MergeItemProperties( oPC, oSheild, oArmorNew );
            }

            if( GetIsObjectValid( oHelm ) ){
                SendMessageToPC( oPC, "<c � >Merging "+GetName( oHelm )+" with "+GetInventorySlotName( INVENTORY_SLOT_CARMOUR )+"!" );
    ...
    if( StringToInt( Get2DAString( "polymorph", "MergeI", nPolymorph ) ) ){
        if( GetIsObjectValid( oArmorNew ) ){
            ...
            if( GetIsObjectValid( oRing1 ) ){
                SendMessageToPC( oPC, "<c � >Merging "+GetName( oRing1 )+" with "+GetInventorySlotName( INVENTORY_SLOT_CARMOUR )+"!" );
                MergeItemProperties( oPC, oRing1, oArmorNew );
            }

            if( GetIsObjectValid( oRing2 ) ){
                SendMessageToPC( oPC, "<c � >Merging "+GetName( oRing2 )+" with "+GetInventorySlotName( INVENTORY_SLOT_CARMOUR )+"!" );
                MergeItemProperties( oPC, oRing2, oArmorNew );
            }

            if( GetIsObjectValid( oAmulet ) ){
                SendMessageToPC( oPC, "<c � >Merging "+GetName( oAmulet )+" with "+GetInventorySlotName( INVENTORY_SLOT_CARMOUR )+"!" );
                MergeItemProperties( oPC, oAmulet, oArmorNew );
            }

            if( GetIsObjectValid( oCloak ) ){
                SendMessageToPC( oPC, "<c � >Merging "+GetName( oCloak )+" with "+GetInventorySlotName( INVENTORY_SLOT_CARMOUR )+"!" );
                MergeItemProperties( oPC, oCloak, oArmorNew );
            }

            if( GetIsObjectValid( oBoots ) ){
                SendMessageToPC( oPC, "<c � >Merging "+GetName( oBoots )+" with "+GetInventorySlotName( INVENTORY_SLOT_CARMOUR )+"!" );
                MergeItemProperties( oPC, oBoots, oArmorNew );
            }

            if( GetIsObjectValid( oBelt ) ){
                SendMessageToPC( oPC, "<c � >Merging "+GetName( oBelt )+" with "+GetInventorySlotName( INVENTORY_SLOT_CARMOUR )+"!" );
                MergeItemProperties( oPC, oBelt, oArmorNew );
            }

            if( GetIsObjectValid( oHands ) ){
                SendMessageToPC( oPC, "<c � >Merging "+GetName( oHands )+" with "+GetInventorySlotName( INVENTORY_SLOT_CARMOUR )+"!" );
