/*
   ...
   Randomly generates a orc invasion encounter on area.
...
void main()
{
    ...
    object oArea = GetAreaByResRef("afr_frontiernort");
    ...
    */
    ...
    // Create the Area below for the invasion
        ...
        {
          oArea = AREAS_CreateArea("afr_frontiernort");
   ...
   //
    ...
    SendMessageToAllDMs("// Launching Orc Invasion Script. Give just over 60 seconds for the entire program to launch. Server Message will announce when ready.");

    DelayCommand(15.0,SendMessageToAllDMs("// Launching Orc PLC Spawns"));

    DelayCommand(30.0,SendMessageToAllDMs("// Launching Orc Mob Spawns"));

    DelayCommand(45.0,SendMessageToAllDMs("// Launching Orc Lieutenant Spawns"));
...
void SpawnMob(object oArea, int nTotalMobs)
{
    ...
    if(GetIsObjectValid(oArea))
    {
      ...
      oCreature1 = CreateObject(OBJECT_TYPE_CREATURE,"af_ds_ork", lRandom, FALSE);
    ...
    if(GetIsObjectValid(oCreature1))
    {
      ...
      oCreature2 = CreateObject(OBJECT_TYPE_CREATURE,"arelithorc001", Location(oArea, vRandomPosN, fFacing), FALSE);
      oCreature3 = CreateObject(OBJECT_TYPE_CREATURE,"arelithorc", Location(oArea, vRandomPosS, fFacing), FALSE);
      oCreature4 = CreateObject(OBJECT_TYPE_CREATURE,"arelithorc", Location(oArea, vRandomPosE, fFacing), FALSE);
      oCreature5 = CreateObject(OBJECT_TYPE_CREATURE,"arelithorc", Location(oArea, vRandomPosW, fFacing), FALSE);
      oCreature6 = CreateObject(OBJECT_TYPE_CREATURE,"orcbasher", Location(oArea, vRandomPosNE, fFacing), FALSE);
      oCreature7 = CreateObject(OBJECT_TYPE_CREATURE,"arelithorc001", Location(oArea, vRandomPosNW, fFacing), FALSE);
      oCreature8 = CreateObject(OBJECT_TYPE_CREATURE,"arelithorc", Location(oArea, vRandomPosSE, fFacing), FALSE);
      oCreature9 = CreateObject(OBJECT_TYPE_CREATURE,"orcbasher", Location(oArea, vRandomPosSW, fFacing), FALSE);
...
void SpawnLieutentant(object oArea, int nTotalLieutenant)
{
    ...
    if(GetIsObjectValid(oArea))
    {
     ...
     oCreature = CreateObject(OBJECT_TYPE_CREATURE,"orcboss001", lRandom, FALSE);
...
void SpawnBoss(object oArea, int nTotalLieutenant)
{
    ...
    oCreature = CreateObject(OBJECT_TYPE_CREATURE,"chosenofkilma002", lRandom, FALSE);
...
void MassWhisper(string sAreaName)
{
  ...
  while(GetIsObjectValid(oPC) == TRUE)
  {
    SendMessageToPC(oPC, "-----");
    SendMessageToPC(oPC, "-----");
    SendMessageToPC(oPC, "News quickly spreads of an amassing army of Orcs in Amia Frontier: North. They must be stopped before it is too late!");
    SendMessageToPC(oPC, "-----");
    SendMessageToPC(oPC, "-----");
    oPC = GetNextPC();
  }
