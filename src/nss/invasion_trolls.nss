/*
   ...
   08/6/19 - Maverick00053

   Randomly generates a troll invasion encounter on area.
...
void main()
{
    ...
    object oArea = GetAreaByResRef("sc_ds_foothills");
    ...
    */
    ...
    // Create the Area below for the invasion
        ...
        {
          oArea = AREAS_CreateArea("sc_ds_foothills");
   ...
   //
    ...
    SendMessageToAllDMs("// Launching Troll Invasion Script. Give just over 60 seconds for the entire program to launch. Server Message will announce when ready.");

    DelayCommand(15.0,SendMessageToAllDMs("// Launching Troll PLC Spawns"));

    DelayCommand(30.0,SendMessageToAllDMs("// Launching Troll Mob Spawns"));

    DelayCommand(45.0,SendMessageToAllDMs("// Launching Troll Lieutenant Spawns"));

    DelayCommand(60.0,SendMessageToAllDMs("// Launching Troll Boss Spawn"));
...
void CreatePRL(location oLoopLocation)
{
   ...
   {
     CreateObject(OBJECT_TYPE_PLACEABLE,"invasioncorpse2", oLoopLocation, FALSE);
   }
   else if(nRandom == 7)
   {
     CreateObject(OBJECT_TYPE_PLACEABLE,"invasiondebris1", oLoopLocation, FALSE);
...
void SpawnMob(object oArea, int nTotalMobs)
{
    ...
    if(GetIsObjectValid(oArea))
    {
      ...
      oCreature1 = CreateObject(OBJECT_TYPE_CREATURE,"mountainguard", lRandom, FALSE);
    ...
    if(GetIsObjectValid(oCreature1))
    {
      ...
      oCreature2 = CreateObject(OBJECT_TYPE_CREATURE,"mountainguard", Location(oArea, vRandomPosN, fFacing), FALSE);
      oCreature3 = CreateObject(OBJECT_TYPE_CREATURE,"mounttroll", Location(oArea, vRandomPosS, fFacing), FALSE);
      oCreature4 = CreateObject(OBJECT_TYPE_CREATURE,"mounttroll", Location(oArea, vRandomPosE, fFacing), FALSE);
      oCreature5 = CreateObject(OBJECT_TYPE_CREATURE,"mounttroll", Location(oArea, vRandomPosW, fFacing), FALSE);
      oCreature6 = CreateObject(OBJECT_TYPE_CREATURE,"mounttroll", Location(oArea, vRandomPosNE, fFacing), FALSE);
      oCreature7 = CreateObject(OBJECT_TYPE_CREATURE,"mounttroll", Location(oArea, vRandomPosNW, fFacing), FALSE);
      oCreature8 = CreateObject(OBJECT_TYPE_CREATURE,"mountainguard", Location(oArea, vRandomPosSE, fFacing), FALSE);
      oCreature9 = CreateObject(OBJECT_TYPE_CREATURE,"mounttroll", Location(oArea, vRandomPosSW, fFacing), FALSE);
...
void SpawnLieutentant(object oArea, int nTotalLieutenant)
{
    ...
    if(GetIsObjectValid(oArea))
    {
     ...
     oCreature = CreateObject(OBJECT_TYPE_CREATURE,"bigmounttroll", lRandom, FALSE);
...
void SpawnBoss(object oArea, int nTotalLieutenant)
{
    ...
    oCreature = CreateObject(OBJECT_TYPE_CREATURE,"invasiontrollbs", lRandom, FALSE);
...
void MassWhisper(string sAreaName)
{
  ...
  while(GetIsObjectValid(oPC) == TRUE)
  {
    SendMessageToPC(oPC, "-----");
    SendMessageToPC(oPC, "-----");
    SendMessageToPC(oPC, "News quickly spreads of an amassing army of Trolls in Skull Crags Foothills. They must be stopped before it is too late!");
    SendMessageToPC(oPC, "-----");
    SendMessageToPC(oPC, "-----");
    oPC = GetNextPC();
  }
