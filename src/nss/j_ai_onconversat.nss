...
void main()
{
    ...
    //undead summon stuff
    if ( GetMaster( OBJECT_SELF ) == oShouter && GetLocalInt( OBJECT_SELF, "is_undead" ) == 1 ){

        if ( GetLastAssociateCommand( OBJECT_SELF ) == ASSOCIATE_COMMAND_FOLLOWMASTER ) {

            SpeakString( "Hurghhh hhuh..." );
            ClearAllActions();
            ActionForceFollowObject( oShouter );
            return;
        }
    }
        else if ( GetLastAssociateCommand( OBJECT_SELF ) == ASSOCIATE_COMMAND_STANDGROUND ) {
