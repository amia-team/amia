...
void main( ){
    ...
    // Customized Death VFX
    int nVFX = GetLocalInt( OBJECT_SELF, "cs_vfx2" );
    int nXPResult;

    if( nVFX > 0 )
        ApplyEffectToObject( DURATION_TYPE_INSTANT, EffectVisualEffect( nVFX ), OBJECT_SELF );

//    LogDebug("j_ai_ondeath", "OnDeath enter for " + GetName(OBJECT_SELF) + " killed by " + GetName(GetLastKiller()));

    // If we are set to, don't fire this script at all
    if(GetAIInteger(I_AM_TOTALLY_DEAD)) {
//        LogDebug("j_ai_ondeath", "OnDeath exit [I_AM_TOTALLY_DEAD]");
        return;
    }
...
//            LogDebug("j_ai_ondeath", "OnDeath exit [EVENT_DEATH_PRE_EVENT]");
            return;
        }
...
//        LogDebug("j_ai_ondeath", "OnDeath exit [AI_TIMER_DEATH_EFFECT_DEATH]");
        return;
    }
...
//        LogDebug("j_ai_ondeath", "OnDeath calling XP_RewardXPForKill");
...
/************************ [Experience] ****************************************/
    ...
    // @@@ AmiaNode: We set NO_COMMONER_ALIGNMENT_CHANGE in j_ai_onspawn
    ...
    // @@@ AmiaNode: We set TURN_OFF_CORPSES in j_ai_onspawn
    //               so this code is unused.
...
//     LogDebug("j_ai_ondeath", "OnDeath calling craft_drop_items");
    /*if( (   GetHasSkill( SKILL_CRAFT_ARMOR, oKiller )   ||
            GetHasSkill( SKILL_CRAFT_WEAPON, oKiller )  )   &&
        d20( ) > 17                                         )*/
     //craft_drop_items    (oKiller);

//     LogDebug("j_ai_ondeath", "OnDeath calling CD_GenerateTreasure");
     GenerateLoot ( OBJECT_SELF, nXPResult );

//     LogDebug("j_ai_ondeath", "OnDeath calling CreateLootableCorpse");
     //CreateLootableCorpse( );

    // Check Trigger Camp
    /*object oCamp = GetLocalObject( OBJECT_SELF, "TriggerCamp" );
...
//    LogDebug("j_ai_ondeath", "OnDeath exit normally");
}
