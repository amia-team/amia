...
void main()
{
    ...
    SetListening(oItem, FALSE);

    string sName = GetLocalString(oItem, "tlr_Spoken");
    sName = BuildLegalString(sName, FALSE, FALSE, FALSE, FALSE, FALSE, FALSE, TRUE);

    SetLocalString(oItem, "CUSTOMNAME", sName);

    if(sName == "___ERROR___")//-- setting = "" not working? delete
    {  DeleteLocalString(oItem, "CUSTOMNAME");  }
