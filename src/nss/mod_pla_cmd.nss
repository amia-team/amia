...
void f_Voice( object oPC, string sOption, string sValue ){
    ...
    if ( sOption == "?" || sOption == "help" ){
        ...
        SendMessageToPC( oPC, "Use:[f_Voice C,F,S,D,B,V,H] [Text]\nOptions: C=companion, F=Familiar, S=summon D=Dominated, B=Bottled Companion, V=Vassal, H=Henchman.\nExample: f_Voice F Donkey Kong\nThis makes your familiar say 'Donkey Kong'." );
        return;
    }
    ...
    if ( sOption == "c" ){
       ...
       if (iMarked == 1){
        ...
        object vassal = GetNearestObjectByTag("vassal_"+GetPCPublicCDKey(oPC), oPC);

        if ( GetIsObjectValid(vassal) && (GetTag(GetHenchman(oPC, 1)) == GetTag(vassal))){
            oTarget = GetHenchman(oPC,2);
        }
        else{
