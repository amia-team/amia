...
{   //SpawnScriptDebugger();
    ...
    if(!GetAssociateState(NW_ASC_IS_BUSY))
    {
        ...
        // check if the master has the assist me flag on from the player tools
        if( GetLocalInt(oMaster, "fw_assist") && GetIsFighting(oMaster) ) {
            object oAssist = GetAttackTarget(oMaster);
            if(oAssist != OBJECT_INVALID) {
                // don't override if the creature is already assisting.
                if( GetAttemptedAttackTarget() != oAssist )  {
                    AssignCommand(OBJECT_SELF, ActionAttack(oAssist));
                    AssignCommand(OBJECT_SELF, DetermineCombatRound(oAssist));
                }
            }
        }
    }
