...
void spellsCure(int nDamage, int nMaxExtraDamage, int nMaximized, int vfx_impactHurt, int vfx_impactHeal, int nSpellID)
{
    ...
    int nExtraDamage = GetNewCasterLevel(OBJECT_SELF); // * figure out the bonus damage
...
void RemoveSpecificEffect( int nEffectTypeID, object oTarget ){
    ...
    while ( GetIsEffectValid( eEffect ) ) {
        ...
        if ( GetEffectType( eEffect ) == nEffectTypeID ) {
        ...
        if(!GetIsReactionTypeFriendly(oTarget))
        {
            ...
            if(o2ndTarget != oTarget)
            {
                ...
                if (nDamage > 0)
                {
                    ...
                    TransferPvpMode( oTarget, oOwner, nTrapStatus, nDamage, "electrical" );
                    //----------------------------------------------------------------
                }
            }
            //Reset the damage
