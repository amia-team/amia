...
// End of Spell Cast Hook
    ...
    // circle vfx
    int nVersus = ALIGNMENT_EVIL;
    int nPulse  = VFX_IMP_PULSE_HOLY;
    int nVFX    = VFX_DUR_PROTECTION_GOOD_MINOR;


    int nTransmutation    = GetLocalInt( OBJECT_SELF, "ds_spell_"+IntToString( SPELL_MAGIC_CIRCLE_AGAINST_GOOD ) );
    if( nTransmutation == 2 )
    {
    nVersus = ALIGNMENT_CHAOTIC;
    nPulse  = VFX_IMP_PULSE_COLD;
    nVFX    = VFX_DUR_AURA_PULSE_GREEN_WHITE;
    }
    // circle vfx
    ...
    while( GetIsObjectValid( oAoeTarget ) )
    {
        ...
        {
        SignalEvent(oAoeTarget, EventSpellCastAt(OBJECT_SELF, SPELL_MAGIC_CIRCLE_AGAINST_EVIL, FALSE));
