...
//check for max bonus to HP roll
    ...
    if( GetIsObjectValid( GetSpellCastItem( ))) {
        HealTarget(GetSpellTargetObject(),nFociBonus, nCasterLevel, nMetaMagic, VFX_IMP_HEALING_L, VFX_IMP_SUNSTRIKE, TRUE);
        return;
    }
    ...
    {
        nFociBonus = 6;
        AoEHeal(nFociBonus, nCasterLevel, nMetaMagic, VFX_IMP_HEALING_L, VFX_IMP_SUNSTRIKE, oSpecificTarget);
    }
    else if (!GetHasFeat (FEAT_EPIC_SPELL_FOCUS_CONJURATION, OBJECT_SELF))
    {
        HealTarget(GetSpellTargetObject(),nFociBonus, nCasterLevel, nMetaMagic, VFX_IMP_HEALING_L, VFX_IMP_SUNSTRIKE);
    }
}
