...
// 2015/10/27 	FW 	    Added duration to buff
...
// End of Spell Cast Hook
    ...
    int nPMLevel = GetLevelByClass( CLASS_TYPE_PALEMASTER, oCaster );
    int nCasterLvl = GetNewCasterLevel(oCaster) + nPMLevel;
    ...
    eGood = EffectLinkEffects(eGood, eVFX);
    ...
    while (GetIsObjectValid(oTarget))
    {
       if(spellsIsTarget(oTarget, SPELL_TARGET_STANDARDHOSTILE, OBJECT_SELF))
       {
            ...
            if (GetRacialType(oTarget) == RACIAL_TYPE_UNDEAD)
            {
                ...
                if( !( GetIsPolymorphed( OBJECT_SELF ) && OBJECT_SELF == oTarget ) ){
                    float fDuration = RoundsToSeconds(nCasterLvl);
                    DelayCommand(fDelay, ApplyEffectToObject(DURATION_TYPE_TEMPORARY, eGood, oTarget, fDuration));
                }
            ...
            if(spellsIsTarget(oTarget, SPELL_TARGET_STANDARDHOSTILE, OBJECT_SELF))
            {
                if(GetRacialType(oTarget) != RACIAL_TYPE_UNDEAD)
                {
                    ...
                    {
                        if(MySavingThrow(SAVING_THROW_WILL, oTarget, GetShifterDC( OBJECT_SELF, GetSpellSaveDC()), SAVING_THROW_TYPE_NEGATIVE, OBJECT_SELF, fDelay))
                        {
                            nDamage /= 2;
                        }
                        //Fire cast spell at event for the specified target
