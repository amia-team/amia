...
//:: Update Pass By: Preston W, On: Aug 3, 200

// 11/7/2016    msheeler    Phantasmal killer: Each Spell Focus adds +1d6 to the
//                          damage on a successful fort save. Even on a successful
//                          Fort save, with epic spell focus, the Target is still
//                          afflicted with the Doom effect for 5 rounds
//                          (-2 attack, damage, saves, skills)
...
// End of Spell Cast Hook
    ...
    int SpellDC = GetSpellSaveDC();

    // Shifter specific
    if( GetIsPolymorphed( OBJECT_SELF ) && GetLevelByClass( CLASS_TYPE_SHIFTER, OBJECT_SELF ) > 0 )
    {
       SpellDC = 10 + 4 + GetAbilityModifier(ABILITY_WISDOM,OBJECT_SELF) + 6; // 10 base, 4 spell level, +wisdom mod, +6 shifter adjustment
    }
    ...
    if(!GetIsReactionTypeFriendly(oTarget))
    {
        ...
        if(!MyResistSpell(OBJECT_SELF, oTarget))
        {
            ...
            if (!MySavingThrow2(SAVING_THROW_WILL,  oTarget, SpellDC, SAVING_THROW_TYPE_MIND_SPELLS))
            {
                // Immunity to fear, makes you immune to Phantasmal Killer.
                if ( GetIsImmune( oTarget, IMMUNITY_TYPE_FEAR ) == FALSE )
                {
                    //Make a Fort save
                    if (MySavingThrow(SAVING_THROW_FORT, oTarget, SpellDC,SAVING_THROW_TYPE_DEATH))
                    {
                         //Check for metamagic
            ...
            if (GetHasFeat(FEAT_EPIC_SPELL_FOCUS_ILLUSION, OBJECT_SELF))
            {
                ApplyEffectToObject (DURATION_TYPE_TEMPORARY, eDoom, oTarget, RoundsToSeconds(5));
            }
        }
    }
}
