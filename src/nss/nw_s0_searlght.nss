    ...
    if(!GetIsReactionTypeFriendly(oTarget))
    {
        ...
        if (!MyResistSpell(oCaster, oTarget))
        {
            ...
            if (GetHasFeat(FEAT_EPIC_SPELL_FOCUS_EVOCATION, OBJECT_SELF))
            {
               ...
               if (GetHasFeat(FEAT_EPIC_SPELL_FOCUS_EVOCATION, OBJECT_SELF) && FortitudeSave(oTarget, GetSpellSaveDC(), SAVING_THROW_TYPE_DIVINE, oCaster) == 0 )
            {
                DelayCommand(0.5, ApplyEffectToObject(DURATION_TYPE_TEMPORARY, eBlind, oTarget, RoundsToSeconds(3)));
             }

            //Check for racial type undead
            if (GetRacialType(oTarget) == RACIAL_TYPE_UNDEAD)
            {
                nDamage = d8(nCasterLevel + nBonusMaxDice);
                nMax = 8;
            }
            ...
            else if (GetRacialType(oTarget) == RACIAL_TYPE_CONSTRUCT)
            {
                ...
                nDamage = d6(nCasterLevel + nBonusMaxDice);
                nMax = 6;
            }
            else
            {
            ...
            {
                nDamage = nMax * (nCasterLevel + nBonusMaxDice);
            }
