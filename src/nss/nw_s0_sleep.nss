...
// End of Spell Cast Hook
    ...
    int nLow;
    // set variable for maximum hd of each critter that can be affected
    int nMaxCritterHD = 6;
    ...
    {
        nDuration1 = nDuration1 * 2;   //Duration is +100%
        ...
        while (GetIsObjectValid(oTarget))
        {
                ...
                && GetRacialType(oTarget) != RACIAL_TYPE_CONSTRUCT && GetRacialType(oTarget) != RACIAL_TYPE_UNDEAD)
            {
                ...
                if (!bAlreadyAffected)
                {
                     ...
                     if(nCurrentHD < nLow && nCurrentHD <= nHD && nCurrentHD < nMaxCritterHD)
                     {
                         nLow = nCurrentHD;
                         oLowest = oTarget;
                         bContinueLoop = TRUE;
            ...
            if (!MyResistSpell(OBJECT_SELF, oLowest))
            {
                ...
                if(!MySavingThrow(SAVING_THROW_WILL, oLowest, GetSpellSaveDC(), SAVING_THROW_TYPE_MIND_SPELLS))
                {
                    ...
                    if (GetIsImmune(oLowest, IMMUNITY_TYPE_SLEEP) == FALSE)
                    {
                        ...
                        ApplyEffectToObject(DURATION_TYPE_TEMPORARY, eLink2, oLowest, RoundsToSeconds(nDuration1));
                        ApplyEffectToObject(DURATION_TYPE_TEMPORARY, eWill, oLowest, RoundsToSeconds(nDuration2));
                    }
                    else
                    // * even though I am immune apply just the sleep effect for the immunity message
                    {
                        ApplyEffectToObject(DURATION_TYPE_TEMPORARY, eSleep, oLowest, RoundsToSeconds(nDuration1));
                        ApplyEffectToObject(DURATION_TYPE_TEMPORARY, eWill, oLowest, RoundsToSeconds(nDuration2));
                    }
