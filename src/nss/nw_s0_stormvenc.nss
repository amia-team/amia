...
void main()
{
    ...
    while(GetIsObjectValid(oTarget))
    {
        if (spellsIsTarget(oTarget, SPELL_TARGET_SELECTIVEHOSTILE, GetAreaOfEffectCreator()))
        {
            ...
            //Make an SR Check
            fDelay = 0.5;//GetRandomDelay(0.5, 2.0);
            if(MyResistSpell(GetAreaOfEffectCreator(), oTarget, fDelay) == 0)
            {
                ...
                if(MySavingThrow(SAVING_THROW_REFLEX, oTarget, GetShifterDC( GetAreaOfEffectCreator() , GetSpellSaveDC() ), SAVING_THROW_TYPE_ELECTRICITY, GetAreaOfEffectCreator(), fDelay))
                {
