...
// End of Spell Cast Hook
    ...
    // Transmutation
    //--------------------------------------------------------------------------
    int nTransmutation    = GetLocalInt( OBJECT_SELF, "ds_spell_"+IntToString( SPELL_WEIRD ) );
    ...
    if( nTransmutation == 2 ){
        ...
        if( nCasterLvl > 20 )
            nCasterLvl = 20;
        eVis = EffectVisualEffect( VFX_FNF_LOS_EVIL_30 );
            ...
            if( GetIsEnemy( oTarget ) ){
                ...
                if(!MyResistSpell( OBJECT_SELF, oTarget, fDelay ) ){
                    ...
                    if(!MySavingThrow(SAVING_THROW_WILL, oTarget, GetSpellSaveDC(), SAVING_THROW_TYPE_FEAR, OBJECT_SELF, fDelay)){
                        ...
                        DelayCommand(fDelay, ApplyEffectToObject( DURATION_TYPE_TEMPORARY, eAbyss, oTarget, RoundsToSeconds( nDur ) ) );
                        DelayCommand(fDelay, ApplyEffectToObject( DURATION_TYPE_TEMPORARY, EffectFrightened(), oTarget, RoundsToSeconds( nDur ) ) );
                    }

                    if(!MySavingThrow(SAVING_THROW_FORT, oTarget, GetSpellSaveDC(), SAVING_THROW_TYPE_NEGATIVE, OBJECT_SELF, fDelay)){

                        eDam = EffectDamage( d8( nCasterLvl ), DAMAGE_TYPE_NEGATIVE );
