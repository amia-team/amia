...
void main()
{
    ...
    //quick check to see if they're already affected, abort if true
    if( GetLocalInt( oTarget, "AuraOfMenace" ) == 1 )
    {
        return;
    }

    //continue to apply effect
    ...
    if(GetIsEnemy(oTarget, GetAreaOfEffectCreator()))
    {
        ...
        if (!/*Will Save*/ MySavingThrow(SAVING_THROW_WILL, oTarget, nDC))
        {
            ...
            //Set variable to prevent duplicate applications
            SetLocalInt( oTarget, "AuraOfMenace", 1 );
            DelayCommand( IntToFloat( nDuration ), DeleteLocalInt( oTarget, "AuraOfMenace" ) );
        }
    }
}
