...
void main()
{
    ...
    if( GetIsPolymorphed( OBJECT_SELF ) && nShape == 39 )
    {
        int nDamage = d6( GetNewCasterLevel( OBJECT_SELF ) / 2 );
        if( nShape == 39 )// custom Risen Thane changes
        {
            if(nTouch > 0)
            {
                if(nTouch == 2)
                ...
                //Set damage effect
                eBolt = EffectDamage(nDamage, DAMAGE_TYPE_PIERCING, DAMAGE_POWER_PLUS_FIVE);
                if(nDamage > 0)
                {
