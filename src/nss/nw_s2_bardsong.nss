// Bard Song
// This spells applies bonuses to all of the bard's allies within 30ft for
// a set duration of 10 rounds.
//
// Revision History
// Date       Name               Description
// ---------- ----------------   ---------------------------------------------
// 02/25/2002 Preston Watamaniuk Initial release.
// 10/01/2003 Georg Zoeller      Losing temporary hp resulted in losing the
//                               other bonuses.
// 04/03/2004 jpavelch           Added EventSpellCastAt to non-party members
//                               to implement Bard Quest #2.
// 09/24/2017 PaladinOfSune      Disabled feat for warlocks
    ...
    while(GetIsObjectValid(oTarget))
    {
        if(!GetHasFeatEffect(FEAT_BARD_SONGS, oTarget) && !GetHasSpellEffect(GetSpellId(),oTarget))
        {
             ...
             if (!GetHasEffect(EFFECT_TYPE_SILENCE,oTarget) && !GetHasEffect(EFFECT_TYPE_DEAF,oTarget))
             {
                ...
                {
                    SignalEvent( oTarget, EventSpellCastAt(OBJECT_SELF, 411, FALSE) );
                }
            }
        }
