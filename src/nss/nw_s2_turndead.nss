...
// Changelog
//-------------------------------------------------------------------------------
//              disco   added Paladin Fall options
//              disco   added "is_undead" toggle to dodge the lack of SetRacialType
//10/7/2011     Selmak  Added turning abilities for new cleric domains
//
...
void main()
{
    ...
    nElemental      += GetHasFeat(FEAT_EARTH_DOMAIN_POWER);
    nElemental      += GetHasFeat(FEAT_FIRE_DOMAIN_POWER);
    nElemental      += GetHasFeat(FEAT_WATER_DOMAIN_POWER);
    nElemental      += GetHasFeat(FEAT_WATER_DOMAIN_POWER);
    nElemental      += GetHasFeat(FEAT_COLD_DOMAIN_POWER);
    ...
    int nReptiles    = GetHasFeat(FEAT_SCALYKIND_DOMAIN_POWER);
    int nShapeChgrs  = GetHasFeat(FEAT_MOON_DOMAIN_POWER);
    int nOutsiders   = GetHasFeat(FEAT_GOOD_DOMAIN_POWER);
    nOutsiders      += GetHasFeat(FEAT_EVIL_DOMAIN_POWER);
    nOutsiders      += GetHasFeat(FEAT_CHAOS_DOMAIN_POWER);
    nOutsiders      += GetHasFeat(FEAT_LAW_DOMAIN_POWER);

    int nPlanar = GetHasFeat(854);

    //Flag for improved turning ability
    int nSun = GetHasFeat(FEAT_SUN_DOMAIN_POWER) + GetHasFeat(FEAT_UNDEATH_DOMAIN_POWER);
...
//##############################################################################
//If a fallen Paladin tries to cast, he will have eLink for the number of levels
//+5 seconds
     ...
     while(GetIsObjectValid(oTarget) && nHDCount < nTurnHD && GetDistanceToObject(oTarget) <= 20.0){
         ...
         if(!GetIsFriend(oTarget))
         {
             ...
             if(nHD <= nTurnLevel && nHD <= (nTurnHD - nHDCount))
             {
                 ...
                 if(nRacial == RACIAL_TYPE_UNDEAD || GetLocalInt( oTarget, "is_undead" ) == 1 )
                 {
                     bValid = TRUE;
                 }
                 ...
                 else if (nRacial == RACIAL_TYPE_OUTSIDER && (nOutsiders+nPlanar > 0) )
                 {
                     bValid = TRUE;
                 }
                 else if ( nRacial == RACIAL_TYPE_HUMANOID_REPTILIAN && nReptiles ){

                    bValid = TRUE;

                 }
                 else if ( nRacial == RACIAL_TYPE_SHAPECHANGER && nShapeChgrs ){
