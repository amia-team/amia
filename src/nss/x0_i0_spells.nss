...
//:: Updated On: July 6th 2006, kfw: Capped Missile storms to 10 missiles per foe.
//:://////////////////////////////////////////////
...
void DoTrapSpike(int nDamage)
{
    ...
    if (nDamage > 0)
    {
        ...
        TransferPvpMode( oTarget, oOwner, nTrapStatus, nDamage, "piercing" );
        //----------------------------------------------------------------
    }
}
//::///////////////////////////////////////////////
        ...
        if(!MyResistSpell(GetAreaOfEffectCreator(), oTarget, fDelay))
        {
           ...
           if (GetHasSpellEffect(SPELL_SPIKE_GROWTH, oTarget) == FALSE)
           {
                ...
                if(!MySavingThrow(SAVING_THROW_REFLEX, oTarget, GetSpellSaveDC(), SAVING_THROW_ALL, GetAreaOfEffectCreator(), fDelay))
                {
                    ...
                    ApplyEffectToObject(DURATION_TYPE_TEMPORARY, eSpeed, oTarget, NewHoursToSeconds(24));
                }
           }
        }
    }
        ...
        if (spellsIsTarget(oTarget, SPELL_TARGET_SELECTIVEHOSTILE, OBJECT_SELF) && (oTarget != OBJECT_SELF) && (GetObjectSeen(oTarget,OBJECT_SELF)))
        {
                ...
                if (!MyResistSpell(OBJECT_SELF, oTarget, fDelay))
                {
                    for (i=1; i <= nNumberOfMissiles; i++)
                    {
                        ...
                        if (GetHasFeat (FEAT_EPIC_SPELL_FOCUS_EVOCATION, OBJECT_SELF) && GetSpellId() == SPELL_CHAIN_LIGHTNING)
                        {
                            effect eReduceElecSave = EffectSavingThrowDecrease (3, SAVING_THROW_TYPE_ELECTRICITY);
                            DelayCommand(fTime, ApplyEffectToObject(DURATION_TYPE_TEMPORARY, eReduceElecSave, oTarget, RoundsToSeconds(5)));
                        }
                        if (GetHasFeat (FEAT_EPIC_SPELL_FOCUS_EVOCATION, OBJECT_SELF) && GetSpellId() == SPELL_FIREBRAND)
                        {
                            effect eReduceFireSave = EffectSavingThrowDecrease (3,SAVING_THROW_TYPE_FIRE);
                            DelayCommand(fTime, ApplyEffectToObject(DURATION_TYPE_TEMPORARY, eReduceFireSave, oTarget, RoundsToSeconds(5)));
                        }
                    }
                } // for
                else
...
void DoPetrification(int nPower, object oSource, object oTarget, int nSpellID, int nFortSaveDC)
{
    ...
    float fDifficulty = RoundsToSeconds(nPower); // One Round per hit-die or caster level
...
void spellApplyMindBlank(object oTarget, int nSpellId, float fDelay=0.0)
{
    ...
    while(GetIsEffectValid(eSearch))
    {
        ...
        else if(GetEffectType(eSearch) == EFFECT_TYPE_FRIGHTENED)
        {
            bValid = TRUE;
        }
...
void doAura(int nAlign, int nVis1, int nVis2, int nDamageType)
{
    ...
    effect eEvil = EffectDamageShield((GetCasterLevel(OBJECT_SELF)/2), DAMAGE_BONUS_1d6, nDamageType);
