...
// 7/18/2016   msheeler   changed to saveless if the spell caster hsa epic spell focus
    ...
    while(GetIsObjectValid(oTarget))
    {
        if (spellsIsTarget(oTarget,SPELL_TARGET_SELECTIVEHOSTILE, OBJECT_SELF))
        {
             ...
             if (!MyResistSpell(OBJECT_SELF, oTarget) )
             {
                ...
                // added byoass to will save if the caster has epic spell focus
                if ((nWillResult == 0) || (GetHasFeat (FEAT_EPIC_SPELL_FOCUS_ENCHANTMENT, OBJECT_SELF)))
                {
