...
//:: Terra: doesnt affect drown immune races
...
// End of Spell Cast Hook
    ...
    string sSub = GetStringLowerCase( GetSubRace ( oTarget ) );

    // creatures with the spell immunity: drown variable are ignored
    // vars
    if(GetLocalInt(
        oTarget,
        "cs_immunity_drown")==1){
    ...
    if(!GetIsReactionTypeFriendly(oTarget))
    {
        ...
        //These races are immune to drown
        if( sSub == "aquatic elf" || sSub == "water genasi" || sSub == "air genasi" )
            return;
