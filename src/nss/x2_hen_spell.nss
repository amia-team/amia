...
void DoAiSpellBehaviour( int nSpell ){

    if( MatchAreaOfEffectSpell( nSpell ) == TRUE ){

        if( GetHasSpell( SPELL_MORDENKAINENS_DISJUNCTION ) > 0 ){

            ActionCastSpellAtLocation( SPELL_MORDENKAINENS_DISJUNCTION, GetLocation( OBJECT_SELF ) );
        }
        else if( GetHasSpell( SPELL_GREATER_DISPELLING ) > 0 ){

            ActionCastSpellAtLocation( SPELL_GREATER_DISPELLING, GetLocation( OBJECT_SELF ) );
        }
        else if( GetHasSpell( SPELL_DISPEL_MAGIC ) > 0 ){

            ActionCastSpellAtLocation( SPELL_DISPEL_MAGIC, GetLocation( OBJECT_SELF ) );
        }
        else if( GetHasSpell( SPELL_LESSER_DISPEL ) > 0 ){

            ActionCastSpellAtLocation( SPELL_LESSER_DISPEL, GetLocation( OBJECT_SELF ) );
        }
    }
    else{

        object oCaster = GetLastHostileActor( );
        //Hostile spells? gear up
        if( GetIsReactionTypeHostile( oCaster ) ){

            if( !GetHasSpellEffect( SPELL_LESSER_SPELL_MANTLE ) && !GetHasSpellEffect( SPELL_SPELL_MANTLE ) && !GetHasSpellEffect( SPELL_GREATER_SPELL_MANTLE ) ){

                if( GetHasSpell( SPELL_LESSER_SPELL_MANTLE ) > 0 ){

                    ActionCastSpellAtObject( SPELL_LESSER_SPELL_MANTLE, OBJECT_SELF );
                }
                else if( GetHasSpell( SPELL_SPELL_MANTLE ) > 0 ){

                    ActionCastSpellAtObject( SPELL_SPELL_MANTLE, OBJECT_SELF );
                }
                else if( GetHasSpell( SPELL_GREATER_SPELL_MANTLE ) > 0 ){

                    ActionCastSpellAtObject( SPELL_GREATER_SPELL_MANTLE, OBJECT_SELF );
                }
            }
            else if( !GetHasSpellEffect( SPELL_GLOBE_OF_INVULNERABILITY ) && !GetHasSpellEffect( SPELL_MINOR_GLOBE_OF_INVULNERABILITY ) ){

                if( GetHasSpell( SPELL_GLOBE_OF_INVULNERABILITY ) > 0 ){

                    ActionCastSpellAtObject( SPELL_GLOBE_OF_INVULNERABILITY, OBJECT_SELF );
                }
                else if( GetHasSpell( SPELL_MINOR_GLOBE_OF_INVULNERABILITY ) > 0 ){
...
void main()
{
    ...
    // * GZ Oct 3, 2003
        ...
        //Terra: DIE!!! DIE!!! DIE!!!
        DoAiSpellBehaviour( GetLastSpell( ) );
