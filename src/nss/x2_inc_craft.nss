...
//:: Reason: See Amia release 1.37 for details.
//::------------------
//:: Updated by: Terra
//:: Update: 2008-03-28
//:: Reason: Seting a name on newly created wands/potions
//::------------------
//:: Updated by: Jes
//:: Update: 4-4-2021
//:: Reason: Stack support for scrolls/wands to match potions
//:://////////////////////////////////////////////
...
//Returns the correct name from a SPELL_* constant
string GetSpellSpellNameFromSpellID(int nSpell);

//---Body
string GetSpellSpellNameFromSpellID(int nSpell)
{
    string sName  = Get2DAString("spells","Name",nSpell);

    return GetStringByStrRef(StringToInt(sName),GENDER_NONE);
}
...
const string  X2_CI_EMPTY_POTION_TAG          = "x2_it_cfm_pbottl";   // Tag for empty bottle
...
int CIGetIsCraftFeatBaseItem(object oItem)
{
    ...
    if (nBt == 101 || nBt == 102 || nBt == 103 || nBt == 104)
      return TRUE;
    else
      return FALSE;
}
...
object CICraftBrewPotion(object oCreator, int nSpellID )
{
    ...
    int nEmptyStack = GetItemStackSize(oSpellTarget);
    ...
    if (nPropID != -1)
    {
        ...
        SetItemStackSize(oTarget,nEmptyStack);
    }
    return oTarget;
}
...
object CICraftScribeScroll(object oCreator, int nSpellID)
{
    //Shameless hack for shades!
    if( nSpellID == 324 || nSpellID == 340 || nSpellID == 341 || nSpellID == 342 || nSpellID == 343 )
        nSpellID = 158;
    ...
    while( sResRef == "" ){
        ...
        sResRef = Get2DAString(X2_CI_2DA_SCROLLS,sClass,nSpellID);
    }
...
int CICraftCheckBrewPotion(object oSpellTarget, object oCaster)
{
    ...
    object oEmptyBottles = GetItemPossessedBy( oCaster, "x2_it_cfm_pbottl");
    int nStackMade = ( GetItemStackSize( oSpellTarget ) );

//    float nExperienceCost = 0.04  * nCost; // xp = 1/25 of gp value
    int nGoldCost = ((nCost / 4) * nStackMade );
    ...
    object oOwner = GetItemPossessor (oSpellTarget);
    // If casting onto an existing stack of potions, no need to create a potion
    if ( GetBaseItemType( oSpellTarget ) == BASE_ITEM_ENCHANTED_POTION ){

        FloatingTextStringOnCreature( "You cannot cast on this item! Cast on unenchanted magical potion bottles!", oOwner ); // Item Creation Failed
        return TRUE;


        }

    else if ( GetBaseItemType( oSpellTarget ) == BASE_ITEM_BLANK_POTION ){
        ...
        // Plot the potion so it can't be sold.
        ...
        {
            // Name the potion with the spell name
            SetName(oPotion, "Potion of "+GetSpellSpellNameFromSpellID(nID));
            TakeGoldFromCreature(nGoldCost, oCaster, TRUE);
            //SetXP(oCaster, nNewXP);
            nEmptyStack = GetItemStackSize( oSpellTarget );

            // If the empty potion stack size is 1, destroy it, else decrease by 1
            //DestroyObject( oSpellTarget );
            SetItemStackSize( oSpellTarget, nEmptyStack );
            DestroyObject(oSpellTarget);
...
int CICraftCheckScribeScroll(object oSpellTarget, object oCaster)
{
    ...
    int nGoldCost = FloatToInt( IntToFloat( nCost ) * 0.95 );
...
*/
    ...
    int nBlankStack = GetItemStackSize( oSpellTarget );
    int nStack = GetItemStackSize( oScroll );
    ...
    if (GetIsObjectValid(oScroll))
    {
        ...
        if ( nBlankStack == 1 ){
            ...
            SetItemStackSize( oSpellTarget, nBlankStack - 1 );
        }
...
int CICraftCheckCraftWand(object oSpellTarget, object oCaster)
{
    ...
    int nGoldCost = FloatToInt( IntToFloat( nCost ) * 0.75 );
...
*/
    ...
    int nBlankWStack = GetItemStackSize( oSpellTarget );
    ...
    {
        TakeGoldFromCreature(nGoldCost, oCaster, TRUE);
        SetName(oWand, "Wand of "+GetSpellSpellNameFromSpellID(nID));
        //SetXP(oCaster, nNewXP);
        ...
        DestroyObject( oSpellTarget );
            ...
            SetItemStackSize( oSpellTarget, nBlankWStack - 1 );
        }
        return TRUE;
        FloatingTextStrRefOnCreature(8502, oCaster); // Item Creation successful
        return TRUE;
...
int CIGetSpellWasUsedForItemCreation(object oSpellTarget)
{
    ...
    else
    {
        ...
        switch (nBt)
        {
                ...
                case 104 :
                            // -------------------------------------------------
                            // Brew Potion (adding to existing stack)
                            // -------------------------------------------------
                           nRet = CICraftCheckBrewPotion(oSpellTarget,oCaster);
                           break;
...
object CIUseCraftItemSkill(object oPC, int nSkill, string sResRef, int nDC, object oContainer = OBJECT_INVALID)
{
    ...
    if (bSuccess)
    {
        ...
        if (bFix)
        {
            object oRet = CopyObjectFixed(oNew,GetLocation(oPC),oPC);
            DestroyObject(oNew);
            oNew = oRet;
        }
