...
//:://////////////////////////////////////////////
//  7/23/2016   msheeler    added bonuns damage for spell foci, increase in AoE and stun duration for Epic focus.
...
void main()
{
    ...
    //--------------------------------------------------------------------------
    // Calculate Damage Dice. 1d per 2 caster levels, max 5d, check for
    // bonus from spell foci
    //--------------------------------------------------------------------------
    ...
    nNumDice = nNumDice + nBonusDice;

    //--------------------------------------------------------------------------
    // Loop through all targets
    //--------------------------------------------------------------------------

    object oTarget = GetFirstObjectInShape(SHAPE_SPHERE, fAoE, lTarget, TRUE, OBJECT_TYPE_CREATURE);
        ...
        if (spellsIsTarget(oTarget, SPELL_TARGET_STANDARDHOSTILE, OBJECT_SELF))
        {
            ...
            if (!MyResistSpell(OBJECT_SELF, oTarget, fDelay))
            {
                ...
                if (nPotential == nDamage || (GetHasFeat(FEAT_IMPROVED_EVASION,oTarget) &&  nDamage == (nPotential/2)))
                {
                    ...
                    {
                        DelayCommand(fDelay, ApplyEffectToObject(DURATION_TYPE_TEMPORARY,eStun,oTarget, RoundsToSeconds(nStun)));
                    }
