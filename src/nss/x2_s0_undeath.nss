...
void DoUndeadToDeath(object oCreature)
{
    ...
    if (!MySavingThrow(SAVING_THROW_WILL,oCreature,GetSpellSaveDC(),SAVING_THROW_TYPE_NONE,OBJECT_SELF))
    {
       ...
       ApplyEffectToObject( DURATION_TYPE_TEMPORARY, EffectSpellResistanceDecrease( nPMLevel ), oCreature, 0.5 );
...
// End of Spell Cast Hook
    ...
    TLVFXPillar(VFX_FNF_LOS_HOLY_20, lLoc,3,0.0f);
     ...
     // calculation
     int nPMLevel = GetLevelByClass( CLASS_TYPE_PALEMASTER, OBJECT_SELF );
     int nLevel = GetCasterLevel(OBJECT_SELF) + nPMLevel;
     if (nLevel>20)
     {
         nLevel = 20;
     }
    ...
    {
        nHDLeft = 4 * nHDLeft;//Damage is at max
    }
