...
//:: 7/9/2016   msheeler    added dex reduction and reflex reduction
...
void main()
{
    ...
    effect eDexReduce = EffectAbilityDecrease (ABILITY_DEXTERITY, 4);
    effect eRefReduce = EffectSavingThrowDecrease (SAVING_THROW_REFLEX, 2);
    effect eLink2 = EffectLinkEffects(eDexReduce,eRefReduce);
    float fDelay;
    ...
    while(GetIsObjectValid(oTarget))
    {
        if(!GetHasFeat(FEAT_WOODLAND_STRIDE, oTarget) &&(GetCreatureFlag(OBJECT_SELF, CREATURE_VAR_IS_INCORPOREAL) != TRUE) )
        {
            if(spellsIsTarget(oTarget, SPELL_TARGET_STANDARDHOSTILE, GetAreaOfEffectCreator()))
            {
                ...
                DelayCommand(fDelay, ApplyEffectToObject(DURATION_TYPE_PERMANENT, eLink2, oTarget));
            }
        }
